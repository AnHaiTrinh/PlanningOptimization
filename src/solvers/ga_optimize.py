...
class Heuristic_GA(Solver):
    # def __init__(self, n, k,distance_matrix, optimal, pop_size=1000, elite_size=1, mutation_rate=0.01, generations=500):
    def __init__(self, n, k,distance_matrix, optimal):
    ...
    def totalDistance(self, inv):
        summ = np.sum([self.distance_matrix[int(inv[i])][int(inv[i+1])] for i in range(2*self.n-1)])
        # print("summ:", summ)
        return summ + self.distance_matrix[0][int(inv[0])] + self.distance_matrix[int(inv[-1])][0]
...
# new_invi1: 1,3,5,2,4  new_invi2: 2,3,5,1,4
    ...
    def mutation(self, father, lenCities=51): # O(N)
    ...
    def GA(self,
        ...
        for _ in range(num_of_generations):
                ...
                if father1 != None:
                    ...
                    if random.random() <= crossover_rate:
                        ...
                        if random.random() <= mutation_rate:
                            inv1 = self.mutation(inv1, lenCities)
                            inv2 = self.mutation(inv2, lenCities)
            ...
            if best_invi[2] > 20*(self.n**0.5):
                return population
            # if (totalDistance(best_invi[1]) < self.min_distance):
            #     self.min_distance = total_distance(best_invi[1])
            # if _ % 100 == 0:
    ...
    def solve(self):
        ...
        for n_seed in range(10):
            ...
            t1 = time.time()
            pop = self.GA(population=population, crossover_func= self.multiPointCrossover, lenCities=len(cities))
        ...
        print("Min distance:", self.min_distance)
        print("Min distance:", type(self.min_distance))
